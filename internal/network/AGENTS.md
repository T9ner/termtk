...
- The `SyncManager` exposes callbacks (`OnMsgRecv`, `OnSearchResult`, `OnOnlineList`, `OnReadAck`) — these run on network goroutines, so keep them non-blocking
- `SendSearchRequest(query)`, `SendWhoOnline()`, and `SendReadAck()` encode frames directly to the relay encoder — call only when relay is online
