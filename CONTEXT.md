...
*   **Read Receipt (read_ack)**: A relay-native frame type sent when a user opens a chat containing unread messages. The recipient's client sends `read_ack` with a batch of message IDs to the relay, which forwards it to the original sender. Upon receipt, the sender updates those messages to `"read"` status. Read receipts are ephemeral — they are not stored for offline recipients.
*   **Unread Count**: The number of messages FROM a contact that have not yet been marked as `"read"`. Displayed as a badge `(N)` next to the contact name in the sidebar. Cleared when the user opens that contact's chat.
*   **Online Presence**: A 30-second periodic poll that calls `who_online` on the relay to refresh a local map of which contacts are currently connected. Combined with local P2P connection state to determine the `[ON]`/`[OFF]` badge in the sidebar.
