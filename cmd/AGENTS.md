...
- [termtalk-relay/main.go](file:///C:/Users/HP/Desktop/termtk/cmd/termtalk-relay/main.go): Relay server — `RelayServer` struct manages client registrations, message routing, store-and-forward for offline recipients, user registry, search, who_online presence, heartbeat keepalive, delete forwarding, graceful shutdown, and periodic health check logging (`ConnectedCount`, `RegisteredCount`, `StoredMessageCount`)
